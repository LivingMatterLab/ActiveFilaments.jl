    ...
    if α2_st isa PiecewiseStructure # Check if helical angle is piecewise in Z
        ...
        for (α2, range) in zip(α2_st.expressions, α2_st.piecewiseRanges)
            if (α2 !== 0 && α2 !== 0.0f0 && α2 !== 0.0) # Helical
                ν = ring.mechanicalProperties.ν;
                δ0 = (ν + 1)*R2^2*cot(α2)^2*log(R2^2*sec(α2)^2/(R1^2*tan(α2)^2 + R2^2)) + ν*(R1^2 - R2^2);
                δ1 = (R1 - R2)*(ν*(R1^2 + R2*R1 + R2^2) - 3*(ν + 1)*R2^2*cot(α2)^2) + 3*(ν + 1)*R2^3*cot(α2)^3*atan(R1*tan(α2)/R2) - 3*α2*(ν + 1)*R2^3*cot(α2)^3;
                δ2 = δ1;
                δ3 = -(1 + ν)*R2*cot(α2)*(R1^2 - R2^2 + R2^2*cot(α2)^2*log(2*R2^2/((R2^2 - R1^2)*cos(2*α2) + R1^2 + R2^2)));
        ...
        if ring.geometry.phi2 == 0 # Non-tapered
            ...
            if (α2 !== 0 && α2 !== 0.0f0 && α2 !== 0.0 && α2 !== -0.0) # Helical
                ...
                δ0 = 2 * (R12 - R22) * ν - (2 * (1 + ν) * log.((tph1 .* ta2) ./ (tph2 .* ta1))) ./ (cpha2);

                δ1 = (2 ./ (cph .* ca .* (cpha2)) 
                        .* (3 * (1 + ν) * (atan.(R1cph) - atan.(R2cph)) .* ca 
                        + (R13 - R23) * ν .* (cph2 .* cph) .* ca
                        + cph .* (-3 * (1 + ν) * (atan.(R1ca) - atan.(R2ca)) + (R23 - R13) * ν .* (ca2 .* ca)))
                        );
                δ2 = δ1;
                δ3 = (1 ./ (cph2 .* ca2 .* sqrt.(Complex.(cpha2)))
                        .* (
                            3 * cph2 .* (atan.((im * R1cph2 + ca) ./ sq1)
                ...
                δ1 = 2 * (R13 - R23) * ν + (6 * (1 + ν) * (atan.(R1cph) - atan.(R2cph) + (R2 - R1) .* cph)) ./ (cph2 .* cph);
                δ2 = δ1;
                δ3 = 0;
            end
        end
...
function computePropertyPrefactors(filament::AFilament)
    ...
    K0 = stiffness.K0; K1 = stiffness.K1; K2 = stiffness.K2; K3 = stiffness.K3;

    prefactors = Vector{Prefactors}();
    for ring in filament.rings
        pre_H = computeHPrefactors(ring);
        push!(prefactors, Prefactors(simplify(pre_H[1] ./ K0), simplify(pre_H[2] ./ K1), simplify(pre_H[3] ./ K2), simplify(pre_H[4] ./ K3)));
    end
...
"""
function computeUQuantities(filament::AFilament{1, M} where M, activationsFourier::Vector{ActivationFourier}, prefactors::Vector{Prefactors}; interp = cubic_spline_interpolation)
    Z = filament.Z;
