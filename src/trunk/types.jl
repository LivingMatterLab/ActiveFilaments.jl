...
@with_kw struct Trunk{T,N}
    ...
    Z::SVector{T,LinRange{Float64,Int64}} =
        SVector{T,LinRange{Float64,Int64}}([LinRange(Z1[i], Z2[i], N) for i = 1:T])
    ...
    R0::SMatrix{T,N,Float64} =
        SMatrix{T,N,Float64}(
            reduce(vcat, [
                transpose(R00 .- Z[i] * tan(φ0)) for i = 1:T
            ]
            ),
        )
    ...
    R1::SMatrix{T,5,SVector{N,Float64}} =
        SMatrix{T,5,SVector{N,Float64}}(
            [
            R10[i, j] .- R10[i, j] / R00 * Z[i] * tan(φ0) for i = 1:T, j = 1:5
        ]
        )
    ...
    R2::SMatrix{T,5,SVector{N,Float64}} =
        SMatrix{T,5,SVector{N,Float64}}(
            [
            R20[i, j] .- R20[i, j] / R00 * Z[i] * tan(φ0) for i = 1:T, j = 1:5
        ]
        )
    ...
    φ2::SMatrix{T,5,Float64} =
        SMatrix{T,5,Float64}(
            [
            atan(R20[i, j] / R00 * tan(φ0)) for i = 1:T, j = 1:5
        ]
        )
    ...
    architectures::SMatrix{T,5,Architecture} =
        SMatrix{T,5,Architecture}(
            reduce(vcat, [
                [longitudinal helical helical radial radial] for i = 1:T
            ]
            ),
        )
    ...
    K0::SMatrix{T,N,Float64} = SMatrix{T,N,Float64}(
        reduce(vcat, [transpose(E * pi * R0[i, :] .^ 2) for i = 1:T]),
    )
    "Trunk stiffness K1"
    K1::SMatrix{T,N,Float64} = SMatrix{T,N,Float64}(
        reduce(vcat, [transpose(E * pi / 4.0 * R0[i, :] .^ 4) for i = 1:T]),
    )
    "Trunk stiffness K2"
    ...
    K3::SMatrix{T,N,Float64} = SMatrix{T,N,Float64}(
        reduce(vcat, [transpose(E * pi / (1.0 + ν) / 4.0 * R0[i, :] .^ 4) for i = 1:T]),
    )
...
@with_kw struct TrunkFast{T,N}
    ...
    interpolations::TrunkInterpolations = TrunkInterpolations(
        SVector{5,PiecewiseFunction{T,Interpolations.Extrapolation}}(
            ...
            PiecewiseFunction(
                SVector{T,Interpolations.Extrapolation}(
                    [cubic_spline_interpolation(trunk.Z[i], trunk.R1[i, j]) for i = 1:T]
                ),
        ...
        SVector{5,PiecewiseFunction{T,Interpolations.Extrapolation}}(
            ...
            PiecewiseFunction(
                SVector{T,Interpolations.Extrapolation}(
                    [cubic_spline_interpolation(trunk.Z[i], trunk.R2[i, j]) for i = 1:T]
                ),
                SVector{T,SVector{2,Float64}}(
                    [SVector{2,Float64}([trunk.Z1[i], trunk.Z2[i]]) for i = 1:T]
                ),
...
@with_kw struct ActivatedTrunkQuantities{T,N}
    ...
    new_K = SVector{4,PiecewiseFunction{T,Interpolations.Extrapolation}}(
        PiecewiseFunction(
            ...
            SVector{T,SVector{2,Float64}}(
                [
                SVector{2,Float64}([trunkFast.trunk.Z1[i], trunkFast.trunk.Z2[i]]) for
                i = 1:T
            ]
            ),
        ),
