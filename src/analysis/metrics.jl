...
function bendingAngle(sol::ODESolution, filament, refVector; largeAngle = false, fullRev = false, n_bending = 20, normal = [1.0, 0.0, 0.0])
    totalBending = estimateTotalBending(sol, filament, refVector, n = n_bending, normal = normal);
...
function tiltAngle(sol::ODESolution, filament, planeNormal; f1 = 1.0 / 5.0, f2 = 2.0 / 5.0, projectionPlaneNormal = [])
...
function tiltAngleCapped(sol::ODESolution, filament, planeNormal; f1 = 1.0 / 5.0, f2 = 2.0 / 5.0, kwargs...)
...
function tiltAngle(r::Vector, L, planeNormal; f1 = 1.0 / 5.0, f2 = 2.0 / 5.0, projectionPlaneNormal = [])
    n = size(r[1])[1];
    Z_range = LinRange(0.0, L, n);
    id1 = n; 
    id2 = findfirst(Z -> Z >= L * (1 - f1), Z_range);
